...
export default function PoliceCasesScreen({ navigation }: PoliceScreenProps<'PoliceCases'>) {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      ...
      case "cloturee": 
        return { color: "#64748B", label: "CLÔTURÉE / ARCHIVÉE", icon: "archive-outline" };
      default: 
        return { color: primaryColor, label: "PROCÉDURE OUVERTE", icon: "folder-open-outline" };
    }
  };
      ...
      >
        <View style={styles.cardHeader}>
          <View style={[styles.idBadge, { backgroundColor: primaryColor + "15" }]}>
            <Text style={[styles.idText, { color: primaryColor }]}>#{item.trackingCode || item.id}</Text>
          </View>
        ...
        <Text style={[styles.title, { color: colors.textMain }]} numberOfLines={1}>
          {item.title || "Dossier d'investigation OPJ"}
        </Text>
    ...
    <ScreenContainer withPadding={false}>
      ...
      <View style={[styles.searchContainer, { backgroundColor: colors.bgCard, borderBottomColor: colors.border }]}>
        <View style={[styles.searchBar, { backgroundColor: colors.inputBg, borderColor: colors.border }]}>
          ...
          <TextInput 
            placeholder="Rechercher (PV, Nom, Motif...)"
            ...
            ListEmptyComponent={
              <View style={styles.emptyContainer}>
                ...
                <Text style={[styles.emptyText, { color: colors.textSub }]}>
                  Aucune procédure n'est actuellement enregistrée sous votre matricule.
                </Text>
              </View>
            }
          />
...
const styles = StyleSheet.create({
  ...
  searchContainer: { paddingHorizontal: 16, paddingVertical: 14, borderBottomWidth: 1 },
  searchBar: { flexDirection: 'row', alignItems: 'center', borderRadius: 14, paddingHorizontal: 12, height: 50, borderWidth: 1 },
  ...
  title: { fontSize: 17, fontWeight: "800", marginBottom: 8, letterSpacing: -0.5 },
