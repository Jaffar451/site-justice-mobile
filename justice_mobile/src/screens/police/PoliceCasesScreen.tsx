...
export default function PoliceCasesScreen({ navigation }: PoliceScreenProps<'PoliceCases'>) {
  ...
  // 🔄 Récupération des dossiers (via React Query)
  ...
  // 🔍 Filtrage dynamique (ID, Titre, Description)
  ...
  /**
   * 🏷️ CONFIGURATION DES BADGES DE STATUT JUDICIAIRE
   */
  const getStatusBadge = (status: string) => {
    switch(status) {
      ...
      case "garde_a_vue":
        return { color: "#EF4444", label: "GARDE À VUE", icon: "lock-closed-outline" };
      default: 
        return { color: "#64748B", label: "ARCHIVÉ / CLOS", icon: "archive-outline" };
    }
  };
      ...
      <View style={{ flex: 1, backgroundColor: colors.bgMain }}>
        {isLoading && !complaints ? (
          <View style={styles.center}>
            ...
            <Text style={[styles.loadingText, { color: colors.textSub }]}>Synchronisation CID...</Text>
          </View>
        ) : (
          <FlatList
            data={filteredCases}
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 16, fontWeight: "900", marginBottom: 8, letterSpacing: -0.4 },
  ...
  chevronBox: { width: 28, height: 28, borderRadius: 14, justifyContent: 'center', alignItems: 'center' },
