...
/**
 * 📡 LOGIQUE DE RECHERCHE CID NIGER
 * Note: À lier plus tard à src/services/arrestWarrant.service.ts
 */
...
export default function PoliceSearchWarrantScreen({ navigation }: PoliceScreenProps<'PoliceSearchWarrant'>) {
  ...
  const renderWarrant = ({ item }: { item: any }) => {
    ...
    return (
      <TouchableOpacity 
        ...
        onPress={() => {
          // Navigation possible vers les détails du dossier RG
          Alert.alert("Dossier Judiciaire", `Accéder au dossier complet ${item.ref} ?`);
        }}
      >
  ...
  return (
    <ScreenContainer withPadding={false}>
      ...
      {/* 🔍 BARRE DE RECHERCHE INSTITUTIONNELLE */}
              ...
              ListEmptyComponent={
                <View style={styles.emptyState}>
                  ...
                  <Text style={[styles.emptySub, { color: colors.textSub }]}>
                    {hasSearched 
                      ? "Aucun titre de contrainte n'a été trouvé pour cette identité au fichier central." 
                      : "Saisissez un nom ou une référence pour vérifier les mandats actifs dans tout le pays."}
                  </Text>
                </View>
              }
            />
